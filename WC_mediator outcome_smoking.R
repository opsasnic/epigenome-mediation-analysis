...
snpmod1 = function(out) {
  temp = na.omit(hrs[,c('WC_new', M,  'cumbur6_zscore2', 'smoking', 'age', 'edu', 'gender', 'indicator2010', 'PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8', 'PC9', 'PC10')])
  MODEL1 = as.formula(parse(text=paste('WC_new', ' ~',M, '+ smoking + cumbur6_zscore2 + age  + gender + edu + indicator2010 + PC1 + PC2 + PC3 + PC4 + PC5 + PC6 + PC7 + PC8 + PC9 + PC10', sep=''))[[1]])
...
path = '/net/orion/skardia_lab/treehouse/science/projects/Health_Retirement_Study/Epigenetics/Stress/results/Aim2/WC/NewAdj/'
...
output = paste0(path, 'HRS.', 'aim2.', 'WCadj.', filenum, '.csv')
